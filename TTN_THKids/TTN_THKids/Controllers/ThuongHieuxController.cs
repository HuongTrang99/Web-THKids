...
namespace TTN_THKids.Controllers
{
    public class ThuongHieuxController : Controller
    {
        ...
        {
            int pagesize = 2;
            ViewBag.tongTrang = db.ThuongHieux.ToList().Count() / pagesize + 1; ;
            var thuongHieu = db.ThuongHieux.Take(pagesize).ToList(); ;
            if (!string.IsNullOrEmpty(index.ToString()))
            {
                thuongHieu = db.ThuongHieux.OrderBy(x => x.MaThuongHieu).Skip(index.Value * pagesize).Take(pagesize).ToList();
